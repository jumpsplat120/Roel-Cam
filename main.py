...
async def main():
    ...
    class CatSpotter(commands.Cog):
        ...
        async def manageStream(self):
            ...
            #If the detected_motion counter is above 0, countdown.
            if self.detected_motion > 0:
                self.detected_motion = self.detected_motion - 1

            #If we're recording, add the color correct frame to the frames array.
            if self.recording:
            ...
            #If detected_motion counted all the way down, then we've
            #finished grabbing all interesting frames.
